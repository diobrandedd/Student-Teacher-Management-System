...
$allowed = ['login','logout','setup','change_password','dashboard','students','student_form','profile','teacher_profile','users','user_form','logs','settings','report','blocks','teachers','courses','departments'];
...
if ($page === 'login' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = db()->prepare("SELECT id, full_name, username, email, password_hash, role, is_active, failed_attempts, locked_until, must_change_password FROM users WHERE username = ? AND role IN ('admin','staff') LIMIT 1");
    ...
    if ($account) {
        ...
        if (!$account['is_active'] || $locked || !password_verify($loginPassword, $account['password_hash'])) {
            ...
            $forceChange = !empty($account['must_change_password']) || hash_equals($loginPassword, default_temp_password());
            $_SESSION['must_change_password'] = $forceChange;
            if ($forceChange && empty($account['must_change_password'])) {
                db()->prepare('UPDATE users SET must_change_password=1 WHERE id=?')->execute([$account['id']]);
            }
            db()->prepare('UPDATE users SET failed_attempts=0, locked_until=NULL, last_login_at=NOW() WHERE id=?')->execute([$account['id']]);
            audit('LOGIN', 'session');
            redirect($forceChange ? 'change_password' : 'dashboard');
        }
    } else {
        ...
        if (!$student || !$student['is_active'] || $hash === '' || $locked || !password_verify($loginPassword, $hash)) {
            ...
            $forceChange = !empty($student['must_change_password']) || hash_equals($loginPassword, default_temp_password());
            $_SESSION['must_change_password'] = $forceChange;
            if ($forceChange && empty($student['must_change_password'])) {
                db()->prepare('UPDATE students SET must_change_password=1 WHERE id=?')->execute([$student['id']]);
            }
...
if ($page === 'change_password') {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($password === default_temp_password()) {
            $changePasswordErrors[] = 'Choose a new password. The temporary password cannot be reused.';
        }
...
if (!in_array($page, ['login','setup','not_found'], true)) require_auth();
...
if ($page === 'students') {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $studentCreateErrors = validate_student($studentInput);
        ...
        if (!$studentCreateErrors) {
            try {
                $sql='INSERT INTO students(student_number,first_name,last_name,email,phone,address,course,course_id,year_level,academic_status,username,password_hash,must_change_password) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,1)';
                ...
                flash('success','Student created. Username is '.$username.'; temporary password is 123. They must change it on first sign-in.'); redirect('students');
...
if ($page === 'student_form') {
    require_role(['admin','staff']); $id=filter_input(INPUT_GET,'id',FILTER_VALIDATE_INT) ?: null; $student=null;
    ...
    $errors=[];
    if ($_SERVER['REQUEST_METHOD']==='POST') {
        verify_csrf(); $data=array_map(fn($v)=>trim((string)$v),$_POST); $errors=validate_student($data); $data['course']=course_name($data['course_id']??null);
        ...
        if (!$errors) {
            try {
                if ($id) {
                    $params = [$data['student_number'],$data['first_name'],$data['last_name'],$data['email'],$data['phone']?:null,$data['address']?:null,$data['course'],$data['course_id'],$data['year_level'],$data['academic_status'],isset($data['is_active'])?1:0,$username];
                    $sql='UPDATE students SET student_number=?,first_name=?,last_name=?,email=?,phone=?,address=?,course=?,course_id=?,year_level=?,academic_status=?,is_active=?,username=?';
                    if ($resetTemp) {
                    ...
                    if ($resetTemp) flash('success', 'Student saved. Username is '.$username.'. Temporary password reset to 123; they must change it on next sign-in.');
                    else flash('success', 'Student record saved. Username is '.$username.'.');
                } else {
                    $sql='INSERT INTO students(student_number,first_name,last_name,email,phone,address,course,course_id,year_level,academic_status,username,password_hash,must_change_password) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,1)';
                    db()->prepare($sql)->execute([$data['student_number'],$data['first_name'],$data['last_name'],$data['email'],$data['phone']?:null,$data['address']?:null,$data['course'],$data['course_id'],$data['year_level'],$data['academic_status'],$username,hash_temp_password()]);
                    $id=(int)db()->lastInsertId(); audit('CREATE','student',$id,'Created student record with login '.$username);
                    flash('success', 'Student created. Username is '.$username.'; temporary password is 123. They must change it on first sign-in.');
                }
                redirect('students');
            } catch (PDOException $exception) {
...
if ($page === 'profile') {
    ...
    $errors=[]; if($_SERVER['REQUEST_METHOD']==='POST'){verify_csrf();$data=['email'=>trim((string)($_POST['email']??'')),'phone'=>trim((string)($_POST['phone']??'')),'address'=>trim((string)($_POST['address']??''))];$errors=validate_student($data,true);if(!$errors){db()->prepare('UPDATE students SET email=?,phone=?,address=? WHERE id=?')->execute([$data['email'],$data['phone']?:null,$data['address']?:null,$student['id']]);audit('UPDATE_PROFILE','student',(int)$student['id']);flash('success','Profile updated.');redirect('profile');}$student=array_merge($student,$data);}
}
...
if ($page === 'users') {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!in_array($role, ['admin','staff'], true)) $createErrors[] = 'Select a valid role.';
        ...
        if (!$createErrors) {
            try {
                db()->prepare('INSERT INTO users(full_name,username,email,password_hash,role,must_change_password) VALUES(?,?,?,?,?,1)')->execute([$fullName,$username,$email,hash_temp_password(),$role]);
                $newId = (int)db()->lastInsertId(); sync_teacher($newId); audit('CREATE','user',$newId,'Created '.$role.' account with temporary password');
                flash('success', (isset($_GET['add_teacher']) ? 'Teacher account created.' : 'User account created.') . ' Share the temporary password privately (123). They must replace it on first sign-in.');
    ...
    $users=db()->query("SELECT id,full_name,username,email,role,is_active,last_login_at,created_at FROM users WHERE role IN ('admin','staff') ORDER BY full_name")->fetchAll();
}
if ($page === 'user_form') {
    require_role(['admin']);
    ...
    $account = null;
    if ($id) {
        $s = db()->prepare("SELECT id,full_name,username,email,role,is_active FROM users WHERE id=? AND role IN ('admin','staff')");
        $s->execute([$id]);
        $account = $s->fetch();
        ...
        if (!$errors) {
            try {
                if ($id) {
                    ...
                    if ($resetTemp) flash('success', 'User saved. Temporary password reset to 123; they must change it on next sign-in.');
                    else flash('success', 'User account saved.');
                } else {
                    db()->prepare('INSERT INTO users(full_name,username,email,password_hash,role,must_change_password) VALUES(?,?,?,?,?,1)')->execute([$fullName, $username, $email, hash_temp_password(), $role]);
                    $id = (int)db()->lastInsertId();
                    audit('CREATE', 'user', $id, 'Created ' . $role . ' account with temporary password');
                    flash('success', 'User account created. Share the temporary password privately (123). They must replace it on first sign-in.');
                }
                sync_teacher((int)$id);
...
if ($page === 'report') { require_role(['admin','staff']);$rows=db()->query('SELECT student_number,last_name,first_name,course,year_level,academic_status FROM students WHERE is_active=1 ORDER BY course,last_name')->fetchAll();if(($_GET['format']??'')==='csv'){audit('EXPORT','student_report',null,'Exported CSV report');header('Content-Type: text/csv; charset=UTF-8');header('Content-Disposition: attachment; filename="student-report.csv"');$out=fopen('php://output','wb');fputcsv($out,array_keys($rows[0]??['student_number'=>'','last_name'=>'','first_name'=>'','course'=>'','year_level'=>'','academic_status'=>'']));foreach($rows as $r)fputcsv($out,$r);fclose($out);exit;} }
if ($page === 'dashboard') { require_auth(); if(user()['role']==='student'){$s=db()->prepare('SELECT s.* FROM students s WHERE s.id=?');$s->execute([user()['id']]);$mine=$s->fetch();$mineBlocks=[];if($mine){$bs=db()->prepare('SELECT b.id, b.name FROM block_students bs JOIN blocks b ON b.id=bs.block_id WHERE bs.student_id=? ORDER BY b.name');$bs->execute([(int)$mine['id']]);$mineBlocks=$bs->fetchAll();}}else{$counts=['students'=>(int)db()->query('SELECT COUNT(*) FROM students WHERE is_active=1')->fetchColumn(),'users'=>(int)db()->query("SELECT COUNT(*) FROM users WHERE is_active=1 AND role IN ('admin','staff')")->fetchColumn(),'courses'=>(int)db()->query('SELECT COUNT(DISTINCT course) FROM students WHERE is_active=1')->fetchColumn()];} }

function render_header(string $title): void {
    global $page;
    $f = take_flash();
    $activePage = ["student_form"=>"students", "user_form"=>"users", "teacher_profile"=>"teacher_profile"][$page] ?? $page;
    $settings = system_settings();
    $schoolName = trim((string)($settings['school_name'] ?? ''));
    $maintenance = trim((string)($settings['maintenance_notice'] ?? ''));
    $role = user()['role'] ?? null;
    ?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title><?=e($title)?><?=$schoolName!==''?' · '.e($schoolName):''?> - SSIS</title><link rel="stylesheet" href="style.css?v=<?=filemtime(__DIR__.'/style.css')?>"><script src="app.js?v=<?=filemtime(__DIR__.'/app.js')?>" defer></script></head><body class="page-<?=e($page)?>"><a class="skip-link" href="#main-content">Skip to main content</a><header><div class="wrap bar"><div class="brand-stack"><?php if(!empty($_SESSION['must_change_password'])):?><a class="brand" href="?page=change_password" aria-current="page">Secure Student IS</a><?php else:?><a class="brand" href="index.php">Secure Student IS</a><?php endif;?><?php if($schoolName!==''):?><span class="brand-school"><?=e($schoolName)?></span><?php endif;?></div><?php if(user()):?><?php if(!empty($_SESSION['must_change_password'])):?><nav aria-label="Main navigation"><a href="?page=logout">Sign out</a></nav><?php else:?><nav aria-label="Main navigation"><a href="index.php" <?=$activePage==="dashboard"?'aria-current="page"':''?>>Dashboard</a><?php if(in_array($role,['admin','staff'],true)):?><span class="nav-group" role="group" aria-label="Records"><a href="?page=students" <?=$activePage==="students"?'aria-current="page"':''?>>Students</a><a href="?page=teachers" <?=$activePage==="teachers"?'aria-current="page"':''?>>Teachers</a><a href="?page=blocks" <?=$activePage==="blocks"?'aria-current="page"':''?>>Blocks</a><a href="?page=report" <?=$activePage==="report"?'aria-current="page"':''?>>Reports</a></span><span class="nav-group" role="group" aria-label="Catalog"><a href="?page=courses" <?=$activePage==="courses"?'aria-current="page"':''?>>Courses</a><a href="?page=departments" <?=$activePage==="departments"?'aria-current="page"':''?>>Departments</a></span><?php endif;?><?php if($role==='student'):?><a href="?page=profile" <?=$activePage==="profile"?'aria-current="page"':''?>>My profile</a><?php endif;?><?php if($role==='staff'):?><a href="?page=teacher_profile" <?=$activePage==="teacher_profile"?'aria-current="page"':''?>>My profile</a><?php endif;?><?php if($role==='admin'):?><span class="nav-group" role="group" aria-label="Administration"><a href="?page=users" <?=$activePage==="users"?'aria-current="page"':''?>>Users</a><a href="?page=logs" <?=$activePage==="logs"?'aria-current="page"':''?>>Audit logs</a><a href="?page=settings" <?=$activePage==="settings"?'aria-current="page"':''?>>Settings</a></span><?php endif;?><a href="?page=logout">Sign out</a></nav><?php endif;?><?php endif;?></div></header><?php if($maintenance!==''):?><div class="notice maintenance-banner" role="status"><?=e($maintenance)?></div><?php endif;?><main id="main-content" class="wrap" tabindex="-1"><div id="modal-status" class="sr-only" role="status" aria-live="polite" aria-busy="false"></div><?php if($f):?><div role="status" class="notice <?=$f[0]==='error'?'error':''?>"><?=e($f[1])?></div><?php endif;?>
<?php }
...
function render_student_fields(array $student, array $courseOptions, array $studentBlocks, bool $isEdit): void {
    ...
    <div class="grid">
      <div><label for="field-student_number">Student number</label><input id="field-student_number" name="student_number" value="<?=e((string)($student['student_number']??''))?>" required inputmode="numeric" pattern="[0-9]{3,30}" maxlength="30"></div>
    ...
    <div class="grid">
      ...
      <div><label for="field-year_level">Year level</label><select id="field-year_level" name="year_level" required><?php for($y=1;$y<=4;$y++):?><option value="<?=$y?>" <?=$year===(string)$y?'selected':''?>><?=$y?></option><?php endfor;?></select></div>
    ...
    <p class="muted">Teachers come from block subject assignments. Manage membership in Blocks.</p>
    <div class="grid">
      <div>
...
if ($page==='login'): ?>
<section class="card narrow"><h1>Secure sign in</h1><p class="muted">Use your authorized account to continue. Accounts are issued by an administrator; there is no self-registration.</p><?php if($loginError):?><div class="notice error" role="alert"><?=e($loginError)?></div><?php endif;?><form method="post"><?=csrf_field()?><label for="field-username">Username</label><input id="field-username" name="username" autocomplete="username" required maxlength="50" spellcheck="false" autocapitalize="none" value="<?=e($loginUsername)?>"><label for="field-password">Password</label><?=password_field('field-password', 'password', ['autocomplete' => 'current-password', 'required' => true])?><div class="actions"><button>Sign in</button><?php if($setupAvailable):?><a href="?page=setup">First-time setup</a><?php endif;?></div></form></section>
...
<section class="card narrow" aria-labelledby="change-password-title">
  ...
  <form method="post" data-change-password-form>
    ...
    <ul id="password-rules" class="password-rules" data-password-rules>
      ...
      <li data-rule="not-temp">Not the temporary password (123)</li>
    </ul>
...
<?php elseif($page==='dashboard'): ?><h1>Dashboard</h1><p>Signed in as <strong><?=e(user()['full_name']??user()['username'])?></strong> <span class="badge"><?=e(role_label(user()['role']))?></span></p><?php if(user()['role']==='student'):?><section class="card"><h2>My academic information</h2><?php if($mine):?><p><strong><?=e($mine['student_number'])?> - <?=e($mine['first_name'].' '.$mine['last_name'])?></strong></p><p><?=e($mine['course'])?>, Year <?=e((string)$mine['year_level'])?> · <?=e($mine['academic_status'])?></p><p>Blocks: <strong><?php if($mineBlocks):?><?=e(implode(', ', array_column($mineBlocks,'name')))?><?php else:?>None yet<?php endif;?></strong></p><?php else:?><p>No student record is linked to your account. Contact the administrator.</p><?php endif;?></section><?php else:?><div class="stats"><?php foreach($counts as $label=>$value):?><section class="card"><div class="stat"><?=$value?></div><div><?=e(ucfirst($label))?></div></section><?php endforeach;?></div><p class="muted">Open Students to search records, Blocks to assign groups, or Reports to export active students.</p><?php endif;?>
...
<div class="bar"><div><h1>Student records</h1><p class="muted">Add the academic record here. Username is auto-built from last name + first initial (temporary password <strong>123</strong>). Teacher logins are under Teachers; staff/admin logins under Users.</p></div><button type="button" data-open-dialog="add-student-dialog">Add student</button></div>
...
<div class="table-wrap"><table>
  ...
  <?php foreach($students as $s):?><tr class="row-link" data-href="?page=student_form&amp;id=<?=$s['id']?>" tabindex="0" aria-label="Edit <?=e($s['first_name'].' '.$s['last_name'])?>"><td><strong><?=e($s['student_number'])?></strong><br><?=e($s['last_name'].', '.$s['first_name'])?></td><td><?=e($s['course'])?> · Year <?=e((string)$s['year_level'])?></td><td><?=($s['block_names']??'')!==''?e((string)$s['block_names']):'<span class="muted">None</span>'?></td><td><?=e($s['academic_status'])?><?php if(!$s['is_active']):?> <span class="badge badge-muted">Deactivated</span><?php endif;?></td></tr><?php endforeach;?>
  </tbody>
</table></div>
...
<?php elseif($page==='student_form'): ?><dialog open class="wide-dialog" aria-labelledby="student-editor-title" data-return-url="?page=students"><div class="bar"><h2 id="student-editor-title"><?=$id?'Edit':'Add'?> student</h2><button type="button" class="icon-button secondary" data-close-dialog aria-label="Close student form">×</button></div><?php errors($errors);?><form method="post" data-student-form><?=csrf_field()?><?php render_student_fields($student ?: [], $courseOptions, $studentBlocks, (bool)$id); ?><div class="actions"><button>Save student</button><?php if($id):?><button type="submit" class="secondary" name="reset_temp_password" value="1">Reset password to 123</button><?php endif;?><button type="button" class="secondary" data-close-dialog>Cancel</button></div></form></dialog>
<?php elseif($page==='profile'): ?><h1>My profile</h1><section class="card"><p><strong><?=e($student['student_number'].' - '.$student['first_name'].' '.$student['last_name'])?></strong></p><p><?=e($student['course'])?>, Year <?=e((string)$student['year_level'])?> · <?=e($student['academic_status'])?></p></section><?php errors($errors);?><form class="card" method="post"><?=csrf_field()?><label for="field-email">Email</label><input type="email" id="field-email" name="email" value="<?=e($student['email'])?>" required><label for="field-phone">Phone</label><input id="field-phone" name="phone" value="<?=e($student['phone'])?>"><label for="field-address">Address</label><textarea id="field-address" name="address"><?=e($student['address'])?></textarea><button>Update profile</button></form>
